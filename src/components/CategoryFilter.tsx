...
const CategoryFilter = ({ activeCategory, setActiveCategory }: CategoryFilterProps) => {
  const categories: Category[] = ['All', 'Vegetable', 'Rice', 'Fruit', 'Breakfast', 'Seafood', 'Fast Food', 'Dessert', 'Beverages'];
      ...
      {categories.map(category => (
        <button
          ...
          className={`px-4 py-2 rounded-full font-semibold text-sm mr-2
            ${activeCategory === category ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
